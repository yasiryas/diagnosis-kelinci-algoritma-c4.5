...
class AuthController extends Controller
{
	...
	{
		if (!$this->config->activeChange) {
			return redirect()->route('user/profile')->with('error', lang('Auth.changeDisabled'));
		}
		return $this->_render($this->config->views['change'], ['config' => $this->config]);
	}

	/**
	 * Attempts to find a user account with that password
	 * and send password change instructions to them.
	 */
	public function attemptChange()
	{
		...
		if (is_null($user)) {
			return redirect()->back()->with('error', lang('Auth.changeNoUser'));
		}
		...
		return redirect()->route('change-password')->with('message', lang('Auth.changeEmailSent'));
	}
	...
	{
		if (!$this->config->activeChange) {
			return redirect()->route('login')->with('error', lang('Auth.changeDisabled'));
		}

		$token = $this->request->getGet('token');

		return $this->_render($this->config->views['change'], [
			'config' => $this->config,
			'token'  => $token,
		]);
	}
